...
3. Writes **execution logs** to a logs file (`{name}-logs.md`)
...
Within a workflow, each step produces three files:
- **Full report:** `{name}.md` - detailed output for next agents
- **Signal file:** `{name}-report.md` - brief status for orchestrator
- **Logs file:** `{name}-logs.md` - execution logs for debugging visibility
...
### Part 3: Execution Logs (`{name}-logs.md`)

Written to `{WORKFLOW_DIR}/{name}-logs.md`. Provides debugging visibility into agent execution.

**Use the log-entry.sh script for consistent formatting:**
```bash
...
**Log Entry Format (generated by script):**
```markdown
## [2024-01-15T10:30:45Z] {ACTION_TYPE}
{Details of what happened}
```

**Action Types:**
| Action Type | When to Log | Example Details |
|------------|-------------|-----------------|
| `STARTED` | At beginning of execution | Agent name, task summary, input files to read |
| `READ_FILE` | After reading each input file | File path, key information extracted |
| `TOOL_CALL` | After using any tool (Bash, Edit, Skill, Task, etc.) | Tool name, parameters, result summary |
| `DECISION` | When making key decisions | What was decided and why |
| `WRITE_FILE` | After writing any file | File path, what was written |
| `COMPLETED` | At successful completion | Final status, files produced |
| `ERROR` | On any error | Error message, attempted recovery |

**IMPORTANT: Always log Skill and Task (agent) invocations:**
- `Skill` tool: Log skill name, arguments, and result
- `Task` tool: Log agent type, task description, whether background

**DO NOT log the log-entry.sh calls themselves** - this would create an infinite loop. Only log substantive tool calls:
- ✅ Log: Bash (npm test, git, etc.), Edit, Write, Read, Skill, Task, Glob, Grep
- ❌ Skip: log-entry.sh calls, wait-for-report.sh calls, remove-report.sh calls

**Example Script Usage:**
```bash
# At start of execution
${CLAUDE_PLUGIN_ROOT}/scripts/log-entry.sh .workflow/task-123/implementation-logs.md STARTED "Agent: backend-developer\nTask: Implement auth feature"

# After reading a file
${CLAUDE_PLUGIN_ROOT}/scripts/log-entry.sh .workflow/task-123/implementation-logs.md READ_FILE "File: requirements.md\nKey points: REQ-1, REQ-2, REQ-3"

# After making a decision
${CLAUDE_PLUGIN_ROOT}/scripts/log-entry.sh .workflow/task-123/implementation-logs.md DECISION "Creating AuthService in src/services/auth.ts\nReason: Following existing pattern"

# After writing code
${CLAUDE_PLUGIN_ROOT}/scripts/log-entry.sh .workflow/task-123/implementation-logs.md WRITE_FILE "File: src/services/auth.ts\nCreated AuthService with login() method"

# After running tests
${CLAUDE_PLUGIN_ROOT}/scripts/log-entry.sh .workflow/task-123/implementation-logs.md TOOL_CALL "Tool: Bash\nCommand: npm test\nResult: 5/5 passing"

# After invoking a skill
${CLAUDE_PLUGIN_ROOT}/scripts/log-entry.sh .workflow/task-123/implementation-logs.md TOOL_CALL "Tool: Skill\nSkill: run-tests\nArgs: tests/auth.test.ts\nResult: 5 tests passed"

# After spawning a subagent
${CLAUDE_PLUGIN_ROOT}/scripts/log-entry.sh .workflow/task-123/implementation-logs.md TOOL_CALL "Tool: Task\nAgent: automation-qa\nTask: Write tests for auth module\nBackground: true"

# After Edit tool
${CLAUDE_PLUGIN_ROOT}/scripts/log-entry.sh .workflow/task-123/implementation-logs.md TOOL_CALL "Tool: Edit\nFile: src/auth.ts\nChange: Added validateToken() method"

# At successful completion
${CLAUDE_PLUGIN_ROOT}/scripts/log-entry.sh .workflow/task-123/implementation-logs.md COMPLETED "Status: DONE\nTests: 5/5 passing"
```

**Example Logs File (generated):**
```markdown

## [2024-01-15T10:30:00Z] STARTED
Agent: backend-developer
Task: Implement auth feature

## [2024-01-15T10:30:05Z] READ_FILE
File: requirements.md
Key points: REQ-1, REQ-2, REQ-3

## [2024-01-15T10:31:00Z] DECISION
Creating AuthService in src/services/auth.ts
Reason: Following existing pattern

## [2024-01-15T10:32:00Z] WRITE_FILE
File: src/services/auth.ts
Created AuthService with login() method

## [2024-01-15T10:33:00Z] TOOL_CALL
Tool: Bash
Command: npm test
Result: 5/5 passing

## [2024-01-15T10:33:30Z] TOOL_CALL
Tool: Skill
Skill: run-tests
Args: tests/auth.test.ts
Result: 5 tests passed

## [2024-01-15T10:34:00Z] TOOL_CALL
Tool: Task
Agent: automation-qa
Task: Write tests for auth module
Background: true

## [2024-01-15T10:34:30Z] TOOL_CALL
Tool: Edit
File: src/auth.ts
Change: Added validateToken() method

## [2024-01-15T10:35:00Z] COMPLETED
Status: DONE
Tests: 5/5 passing
```
...
- Verify agents are reading correct input files
- Understand decision-making process
- Track which tools were used and their results
- Identify where execution stalled or failed
