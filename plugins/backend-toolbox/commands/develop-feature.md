...
**If BLOCKING issues found:** 3. Pass the BLOCKING findings to `backend-developer` with: - The specific performance issues identified - The suggested fixes from `performance-specialist` - Relevant `/run-tests` invocations to verify behavior is preserved 4. `backend-developer` implements fixes (small steps, run `/run-tests` after each step) 5. **Go back to Loop start** — re-invoke `performance-specialist` for a new review
...
**If BLOCKING issues found:** 3. Pass the BLOCKING findings to `backend-developer` with: - The specific security vulnerabilities identified - The recommended fixes from `application-security-specialist` - Relevant `/run-tests` invocations to verify behavior is preserved 4. `backend-developer` implements fixes (small steps, run `/run-tests` after each step) 5. **Go back to Loop start** — re-invoke `application-security-specialist` for a new review
